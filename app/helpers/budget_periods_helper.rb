module BudgetPeriodsHelper
  ...
  def budget_period_show_nav_links(budget_period)
    [
      ...
      { body: t('views.transactions.add_income'), path: new_budget_income_transaction_path(budget_period.uid) },
      { body: t('views.transactions.add_expense'), path: new_budget_expense_transaction_path(budget_period.uid) }
    ]
  end
